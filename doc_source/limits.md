...
| Maximum number of anomaly types in a project | 100 | 
| Maximum number of anomaly types on a mask image | 20 | 
| Minimum number of images for an anomaly type\. To count, the image must contain only one type of anomaly\.  | 20 for a single dataset project\. 10 for each dataset in a project with separate training and test datasets\. | 
